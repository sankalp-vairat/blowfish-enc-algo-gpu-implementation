...
#
def encryption(data_l, data_r):

    for i in range (0, N, 1):
        data_l = data_l ^ P[i]
        data_r = fiestel_function(data_l) ^ data_r
        temp = data_l
        data_l = data_r
        data_r = temp
...
def sub_key_generation(key):
    ...
    for i in range(0, N+2, 2):
        data_l, data_r = encryption(data_l, data_r)
        P[i] = data_l
        P[i+1] = data_r
...
def make_64_bit(size):
    char_to_append=0
    if(size%8!=0):
        mod_64=(size%8)
        print mod_64
        char_to_append=8-mod_64
...
string_append=make_64_bit(size)
...
lengthI = len(fileI)
print "length",lengthI
start_time = time.time()

for i in range(0, lengthI, 2):
    fileI[i], fileI[i+1] = encryption(fileI[i], fileI[i+1])

fileI.tofile(encrypted_file_name)

total_time = time.time() - start_time

print("Encryption time %fs" % total_time)

print "before fileI",fileI
...
# for i in range(0, lengthI, 2):
    # fileI[i], fileI[i+1] = decryption(fileI[i], fileI[i+1])


# total_time = time.time() - start_time

# print("Decryption time %fs" % total_time)

# print "fileI",fileI

# #--------------------------------------------------------------------------

# size_fileI=len(fileI)

# second_last_block=fileI[size_fileI-2]
# last_block=fileI[size_fileI-1]
...
# fileI.tofile(decrypted_file_name)
# size=os.path.getsize(decrypted_file_name)

# with open(decrypted_file_name, 'rb+') as f:    #read binary
    # f.seek(0,2)                                #1-seek relative to current position . 2- seek relative to file's end.
    # size=f.tell()
