...
@cuda.jit('void(uint32[:],uint32[:],uint32[:],uint32[:])',type="gpu")
def encryption(data_l,data_r,s,p):
    ...
    for i in range(16):
        if(idx < len(data_l)):
            ...
            y = y ^ s[512+c]
            y = y & s[768+d]
            data_r[idx] = y ^ data_r[idx]
            cuda.syncthreads()
            temp = data_l[idx]
            data_l[idx] = data_r[idx]
            cuda.syncthreads()
            data_r[idx] = temp
            cuda.syncthreads()
    ...
    if(idx < len(data_l)):
        data_r[idx] = data_r[idx] ^ p[N]
        cuda.syncthreads()

    if(idx<len(data_l)):
        data_l[idx] = data_l[idx] ^ p[N + 1]
        cuda.syncthreads()
 
@cuda.jit('void(uint32[:],uint32[:],uint32[:],uint32[:])',type="gpu")
def decryption(data_l,data_r,s,p):
    tid = cuda.threadIdx.x
    bid=cuda.blockIdx.x
    bdm=cuda.blockDim.x
    ...
    if(idx < len(data_l)):
        data_r[idx] = data_r[idx] ^ p[1]
        cuda.syncthreads()
    if(idx<len(data_l)):
        data_l[idx] = data_l[idx] ^ p[0]
        cuda.syncthreads()


def fiestel_function(key_l):
    d = key_l & 0x00FF 
    key_l >>= 8
    c = key_l & 0x00FF
    key_l >>= 8
...
def Blowfish_encipher(key_l, key_r):

    for i in range (0, N, 1):
        key_l = key_l ^ P[i]
        key_r = fiestel_function(key_l) ^ key_r
        temp = key_l
        key_l = key_r
        key_r = temp
...
def Blowfish_decipher(key_l, key_r):
...
def sub_key_generation(key):
    ...
    for i in range(0, N+2, 2):
        data_l, data_r = Blowfish_encipher(data_l, data_r)
        P[i] = data_l
        P[i+1] = data_r
...
def key_expansion(key):
    ...
    key_mat=[0]*14
    j=0
    for i in range(0,len(new_key),4):
        str1=bin(ord(new_key[i]))[2:].rjust(8,'0')+bin(ord(new_key[i+1]))[2:].rjust(8,'0')+bin(ord(new_key[i+2]))[2:].rjust(8,'0')+bin(ord(new_key[i+3]))[2:].rjust(8,'0')
        key_mat[j]=int(str1,2)
        j=j+1

    key=np.asarray(key_mat, dtype='uint32')
    return key
...
#decrypted_file_name=file_name+"_decr"
...
if(size%8!=0):
    mod_8=(size%8)
    print mod_8
    char_to_append=8-mod_8


print "size",size
char_to_append=char_to_append+8
print "char_to_append",char_to_append

string_append=""
for i in range(char_to_append):
    if(char_to_append-8==i):
        string_append=string_append+chr(char_to_append-8)
    else:
        string_append=string_append+':'

print "string_append",string_append

f = open(file_name, 'a')
f.write(string_append)
f.close()
...
try:
    f = open(file_name, 'rb')
    file_array = np.fromfile(f, dtype='uint32')
    f.close()
...
lengthI = len(file_array)

print "file_arraylength",lengthI
...
with open(file_name, 'rb+') as f:
    f.seek(0,2)
    size=f.tell()
    f.truncate(size-len(string_append))


file_array_odd=np.uint32([1]*(lengthI/2))

file_array_even=np.uint32([0]*(lengthI/2))

j=0
for i in range(0,len(file_array),2):
    file_array_odd[j]=file_array[i+1]
    file_array_even[j]=file_array[i]
    j=j+1
...
Text_1 = cuda.to_device(file_array_even)
Text_2 = cuda.to_device(file_array_odd)
dS = cuda.to_device(S1)
dP = cuda.to_device(P)

encryption[bpg, tpb](Text_1,Text_2, dS,dP)
file_array_even=Text_1.copy_to_host()
file_array_odd=Text_2.copy_to_host()

print "file_array_even",file_array_even
print "file_array_odd",file_array_odd

j=0
for i in range(0,len(file_array),2):
    file_array[i]=file_array_even[j]
    file_array[i+1]=file_array_odd[j]
    j=j+1

print "file_array",file_array
total_time = time.time() - start_time
print("Encryption takes %fs" % total_time)
file_array.tofile(encrypted_file_name)


#print "file_array befre",file_array
lengthI=len(file_array)

address="10.100.53.87"
port=5000
sock_Obj=socket.socket()
sock_Obj.connect((address,port))
size=os.path.getsize(encrypted_file_name)
str1=file_name+":"+str(size)
print "Sending file name:"
sock_Obj.send(str1);
reply=sock_Obj.recv(1024)

if(reply=="ok"):
	print "ACK received. Sending file."
	total=0
...
# decryption[bpg, tpb](Text_1,Text_2, dS,dP)
# file_array_even=Text_1.copy_to_host()
# file_array_odd=Text_2.copy_to_host()
...
# size_file_array=len(file_array)

# print "file_array_len",size_file_array

# print "file_array after",file_array

# second_last_block=file_array[size_file_array-2]
# last_block=file_array[size_file_array-1]

# second_last_block=bin(second_last_block)
# bin_last_block=second_last_block[2:]

# str1=""
# for i in range(len(bin_last_block)-1,len(bin_last_block)-5,-1):
    # str1=bin_last_block[i]+str1


# prune_chr=int(str1,2)

# print "prune_chr",prune_chr
# file_array.tofile(decrypted_file_name)

# with open(decrypted_file_name, 'rb+') as f:
    # f.seek(0,2)
    # size=f.tell()
    # f.truncate(size-(prune_chr+8))