import numpy as np
from numba import cuda,SmartArray
import time 
import os
import socket
...
print "hello",size-len(string_append)
...
p_smart=SmartArray(P,where='host',copy=True)
s_smart=SmartArray(S1,where='host',copy=True)
file_array_even_smart=SmartArray(file_array_even,where='host',copy=True)
file_array_odd_smart=SmartArray(file_array_odd,where='host',copy=True)


print "file_array_even",file_array_even_smart.__array__()
print "file_array_odd",file_array_odd_smart.__array__()


cfunc = cuda.jit()(encryption)
cfunc[bpg, tpb](file_array_even_smart,file_array_odd_smart,s_smart,p_smart)


j=0
for i in range(0,len(file_array),2):
    file_array[i]=file_array_even_smart[j]
    file_array[i+1]=file_array_odd_smart[j]
    j=j+1
